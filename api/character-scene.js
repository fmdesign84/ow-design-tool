...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        if (scene === 'marathon-start' && scenePreset) {
            ...
            preset = {
                ...scenePreset,
                scene: `A wide city road at the starting line of a marathon race. There is exactly ONE single start gate arch spanning across the road. This is ONE inflatable arch gate (not two separate structures). The arch is WHITE with a puffy inflatable look. "${bannerText}!" is written in VERY BOLD, large, thick dark letters on the top beam of the arch. The character is standing underneath this single arch gate, with the empty road stretching ahead. Camera is positioned behind the gate looking through it. Bright daytime, clear blue sky with warm sunlight. Urban cityscape with buildings and trees lining the street. IMPORTANT: Only ONE start gate/arch in the entire image. Do NOT generate two gates or two balloon arches.`,
            };
        ...
        // 전광판 씬: 동적 텍스트 생성
            ...
            preset = {
                ...scenePreset,
                scene: `A city marathon road in bright daytime with a MASSIVE wide HORIZONTAL RECTANGULAR LED electronic billboard/jumbotron screen mounted on the right side (landscape orientation, much wider than tall, like a widescreen TV). The billboard is a clean RECTANGLE shape with sharp corners and a thin black frame border. On the billboard screen with dark/black background: (1) the name "${nameText}!" displayed VERY LARGE in clean WHITE text with a subtle soft LED glow effect - simple, modern, elegant typography (NOT bold/thick/neon, just clean white with gentle luminous glow like a real LED display), (2) below it in a smaller white subtitle: "YOU MADE FOREST!". The billboard also shows the same mint-green-haired 3D cartoon character running. The billboard blends naturally into the scene. Clear blue sky, urban atmosphere, sunlight.`,
            };
            outfitDescription = OUTFIT_PRESETS[scenePreset.outfit] || OUTFIT_PRESETS['default'];
