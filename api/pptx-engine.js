/**
 * PPTX Engine - OOXML 직접 조작 기반 PPT 생성 엔진
 *
 * 라이브러리 한계 없이 완전한 미세 제어 가능
 */
...
function createTextElement(el, id) {
  ...
  const align = el.align || 'l'; // l, ctr, r
  const valign = el.valign || 't'; // t, ctr, b
...
/**
 * 템플릿 기반 PPTX 생성
 * @param {Object} template - 슬라이드 템플릿
 * @param {Buffer} baseTemplate - 기본 PPTX 템플릿 (선택)
 */
async function generatePptx(template, baseTemplate = null) {
  let zip;

  if (baseTemplate) {
    // 기존 템플릿 수정
    zip = await JSZip.loadAsync(baseTemplate);
  } else {
    // 새로 생성 (기본 구조 필요)
    zip = await createBasePptx();
  }
